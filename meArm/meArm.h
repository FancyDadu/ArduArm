...
class meArm {
  public:
    ...
    meArm(int sweepMinBase = 145, int sweepMaxBase = 49, float angleMinBase = -pi / 4, float angleMaxBase = pi / 4,
          ...
          int sweepMinGripper = 130, int sweepMaxGripper = 173, float angleMinGripper = pi / 2, float angleMaxGripper = 0);
    //required before running
    void begin(int pinBase, int pinShoulder, int pinElbow, int pinGripper);
	
	void rest();
	void setRest(short x,short y,short z);
	void getUp();
  ...
  private:
    ...
    float _x, _y, _z;
	short _rx=0,_ry=150,_rz=40; //resting positions
    float _r, _t;
